#!/usr/bin/env python3
"""Static validation for FS25_PhobosLib.
...
def validate_moddesc(mod_root: Path, validation: Validation) -> None:
    ...
    if author != "phobosgekko":
        validation.error(f"Expected author 'phobosgekko', found '{author}'")

    title = (root.findtext("title/en") or "").strip()
    if title != "Phobos FS25 Library":
    ...
    if dependencies:
        validation.error("FS25_PhobosLib must remain dependency-free")
    ...
    source_files = [
        node.get("filename", "").strip()
        for node in root.findall("./extraSourceFiles/sourceFile")
        if node.get("filename", "").strip()
    ]
    if "src/PhobosFS25.lua" not in source_files:
        validation.error("modDesc.xml must load src/PhobosFS25.lua")

    for filename in source_files:
        if not (mod_root / filename).is_file():
...
def validate_lua_namespace(mod_root: Path, validation: Validation) -> None:
    namespace_path = mod_root / "src" / "PhobosFS25.lua"
    if not namespace_path.is_file():
        validation.error(f"Missing namespace file: {namespace_path}")
        return

    text = namespace_path.read_text(encoding="utf-8")
    if "PhobosFS25" not in text:
        validation.error("src/PhobosFS25.lua must define the PhobosFS25 namespace")
...
def validate_source(repo_root: Path, mod_source: str, validation: Validation) -> None:
    ...
    validate_lua_namespace(mod_root, validation)
...
def validate_package(package_path: Path, validation: Validation, soft_size_limit: int) -> None:
    ...
    for name in names:
        ...
        if name in FORBIDDEN_PACKAGE_FILES:
            validation.error(f"Package contains repository-only file: {name}")


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate FS25_PhobosLib source or package")
    parser.add_argument("--repo-root", default=".", help="Repository root")
    parser.add_argument("--mod-source", default=".", help="Mod source folder relative to the repository root")
