...
$modDescPath = Join-Path $repoRoot "modDesc.xml"
...
$assetPath = Join-Path $distRoot "FS25_PhobosLib_v$Version.zip"
...
try {
    ...
    if (-not [string]::IsNullOrWhiteSpace($NotesFile)) {
        ...
        $notes = @"
Package: FS25_PhobosLib_v$Version.zip
...
"@
    ...
    git tag -a $tag -m "FS25_PhobosLib $tag"
    git push origin $tag
